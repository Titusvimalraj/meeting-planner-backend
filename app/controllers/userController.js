                        ...
                        newUser.save((err, newUser) => {
                            if (err) {
                                ...
                                let sendEmailOptions = {
                                    ...
                                    You have Successfully signed up with Meeting Planner!.                                     
                                    `
                                }

                                setTimeout(() => {
                                    emailLib.sendAnEmail(sendEmailOptions);
                                }, 2000);
...
/**
 * @author Titus Vimal Raj
 * @description function to get User Info.
 * @param {String} userId
 */

let userInfo = (req, res) => {
    UserModel.findOne({ userId: req.params.userId }).select('-_id -__v -password').exec((err, details) => {
        if (err) {
            logger.error('Error in fetching user information', 'UserController:userInfo()', 1);
            let apiResponse = responseLib.generateResponse(true, 'Internal server error occured while fetching user information', 500, null);
            res.status(apiResponse.status).send(apiResponse);
        }
        else if (checkLib.isEmpty(details)) {
            logger.info('Requested user is not found', 'UserController:userInfo()', 2);
            let apiResponse = responseLib.generateResponse(true, 'User with this email not found Kindly sign up to continue ', 201, null);
            res.status(apiResponse.status).send(apiResponse);
        }
        else {
            logger.info('User Found', 'UserController:userInfo()', 1);
            let apiResponse = responseLib.generateResponse(false, 'User found', 200, details);
            res.status(apiResponse.status).send(apiResponse);
        }
    })
...
let getAllUsers = (req, res) => {
    UserModel.find().select('-_id -__v -password').exec((err, details) => {
        if (err) {
            logger.error('Error in fetching users information', 'UserController:getAllUsers()', 1);
            let apiResponse = responseLib.generateResponse(true, 'Internal server error occured while fetching user information', 500, null);
            res.status(apiResponse.status).send(apiResponse);
        }
        else if (checkLib.isEmpty(details)) {
            logger.info('Users not found', 'UserController:getAllUsers()', 2);
            let apiResponse = responseLib.generateResponse(true, 'kindly check the Database for users ', 201, null);
            res.status(apiResponse.status).send(apiResponse);
        }
        else {
            logger.info('User Found', 'UserController:getAllUsers()', 1);
            let apiResponse = responseLib.generateResponse(false, 'Users found', 200, details);
            res.status(apiResponse.status).send(apiResponse);
        }
    })
}
